...
Downloaded the diversity data from the PREDICTS database (diversity-2018-03-28-02-36-24.rds). This will include all the diversity data availalble on the database, but I need to subset to include only birds, so I subset for the taxonomical Class Aves.
```{r results="hide"}
diversity<- readRDS("../../RawData/diversity-2018-03-28-02-36-24.rds")
``` 
## Exploring the data
...
sum(diversity$Class== "") # this is to check for empty cells. There are 32782 cell without class
...
sum(birds$Best_guess_binomial== "") #this check for empty, there are 6135 records that don't have best guess binomial but only have higher level taxonomy (i.e.family or genus only)
sum(birds$Family== "") # 694 records don't have the family level
sum(birds$Order =="")
# 492 records don't have Order level. so to keep in mind in case they can be included in some higher taxonomy level analysis
...
### Aves data
There are  411138 Aves in the PREDICTS database
Best_guess_binomial field:

* 6135 records that don't have best guess binomial but only have higher level taxonomy
(at this stage i don't need to worry about removing the 6135 records as they will be dropped when i run SSI)
...
Removing the CANNOT DECIDE brings the records to 249624

```{r results="hide", message=FALSE, warning=FALSE}
# to check what predominant habitat categories I have in the data set
  summary(birds$Predominant_habitat)
# should get rid of the "Cannot decide" as they get categorised as "converted"
...
I am now calculating the SSI for 2126 species and plotting the values
...
I now load the traits data for all the birds species for which i calculated the SSI
So I have the trait data for the 2161 species with a value of SSI.
```{r, message=FALSE, warning=FALSE}
...
* At this point I have 2134 species (birdstraitdata dataset)
...
# there are 133 species that don't match between the two columns, need to see why and which one of the two to use
